...
// sanitize_email($string) -- input string, returns string stripped of all non
// sanitize_domain($string) -- input string, returns string stripped of all non
...
function sanitize_email($string, $min='', $max='')
{
  $string = preg_replace("/[^a-zA-Z0-9._@]/", "", $string);
  $len = strlen($string);
  if((($min != '') && ($len < $min)) || (($max != '') && ($len > $max)))
...
function sanitize_domain($string) {
	str_replace("http://", "", $string);
  	$string = preg_replace("/[^a-zA-Z0-9.\-\_\/\:]/", "", $string);
  	$len = strlen($string);
  	return $string;
}

function sanitize_phone($string) {
	$pattern = "/./";
  	$replacement = "-";
...
function sanitize_zipcode($string)
{
  $string = preg_replace("/[^0-9\-]/", "", $string);
  $len = strlen($string);
  if ($len > 10) {
...
function sanitize_html_sql($string) {
	$pattern = "/;/";
  	$replacement = "";
 	$string =  preg_replace($pattern, $replacement, $string);
...
function cleanInput() {
	foreach ($_POST as $key => $val) {
		if ($key == "url") {
			$_POST[$key] = sanitize_domain($val);
		}
		elseif (($key == "description") || ($key == "discount")) {
			// No Sanitization
		}
		/*elseif ($key == "comments") {
			$_POST[$key] = sanitize_html_sql(filter_bad_words($val));
		}*/
		else {
			$_POST[$key] = sanitize_html_sql($val);
		}
	}
}


// Converts html elements back to original form (make sure nothing gets interpretted!)
